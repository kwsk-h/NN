...
int main(void)
{
	...
	= { { "data\\lea_sig.csv","data\\lea_T_sig.csv" } };
	//select dataset
	int dataset_num = 1;
	cout<<"-----read files-----" << endl;
	fileset(datasets[dataset_num - 1][0], input_datas);
	fileset(datasets[dataset_num - 1][1], input_labels);	
	//input_datas = { {0,0,0,},{1,0,1},{1,1,1},{1,1,0},{1,0,0},{0,0,1} }; //XOR data
	...
	int batch_size = 20;			//batchƒTƒCƒY‚Ìw’è(1©online, N©batch, n©mini batch)
